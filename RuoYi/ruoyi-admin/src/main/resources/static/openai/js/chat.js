          ...
          >
            ...
            >
              <div class="flex-shrink-0 w-8 h-8">
                <img class="w-full h-full" src="/img/head.jpeg" />
              </div>
              <div class="marked" >
                  ${content}
              </div>
...
function connect() {
    ...
    socket = io.connect('http://192.168.99.99:9099/', opts);
    // socket = io.connect('http://chatgpt.byabstudio.com/', opts);
...
let pageSize = 10;
let count = 1;
getMessage(pageNum,pageSize,"init")
function getMessage(pageNum,pageSize,init) {
    let formData = `pageNum=${pageNum}&pageSize=${pageSize}&orderByColumn=id&isAsc=desc`
    fetch('/system/detail/list',{
        method:"post",
        headers:{
        ...
        count = Math.ceil(res2.total/pageSize);
    // 进行页面的渲染
...
$("#messageContent").scroll(function (event) {
    ...
    if (event2==0) {
        if(pageNum>=count){
            ...
            pageNum+=1;
            getMessage(pageNum,pageSize,"orther")
        }
    }
})